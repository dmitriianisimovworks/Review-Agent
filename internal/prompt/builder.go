...
Стремись вернуть 3-5 независимых и действительно сильных замечаний, если в текущем фрагменте для твоей роли достаточно оснований.
...
- целевой объём ответа на один фрагмент: 3-5 findings;
...
`, roleDisplayName(role), roleInstructions(role), role))
	...
	user := fmt.Sprintf(
		"Режим анализа: %s\nИсточник документа: %s\nНазвание документа: %s\nРоль ревью: %s\nФрагмент: %d из %d\n%s%s\nПроведи ревью следующего фрагмента технической спецификации и, если в нём достаточно оснований, верни 3-5 независимых замечаний в JSON. Если сильных проблем меньше, верни меньше. Верни только один JSON-объект. Начни ответ сразу с {\"findings\": [...]}.\n\n%s",
		mode,
