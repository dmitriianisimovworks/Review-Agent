...
Стремись вернуть 3-4 независимых и действительно сильных замечания, если в текущем фрагменте для твоей роли достаточно оснований.
...
- если в текущем фрагменте реально есть достаточно независимых проблем по твоей роли, старайся вернуть не меньше 3 findings;
- целевой объём ответа: 3-4 findings;
- findings должно быть не больше 6;
...
`, roleDisplayName(role), roleInstructions(role), role))
	...
	user := fmt.Sprintf(
		"Режим анализа: %s\nИсточник документа: %s\nНазвание документа: %s\nРоль ревью: %s\nФрагмент: %d из %d\n%s%s\nПроведи ревью следующего фрагмента технической спецификации и, если в нём достаточно оснований, верни 3-4 независимых замечания в JSON. Если сильных проблем меньше, верни меньше. Верни только один JSON-объект. Начни ответ сразу с {\"findings\": [...]}.\n\n%s",
		mode,
